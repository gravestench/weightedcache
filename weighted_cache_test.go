...
func TestCacheInsert(t *testing.T) {
	...
	if insertError != nil {
		t.Fatalf("WeightedCache insert resulted in unexpected error: %s", insertError)
	}
}
...
func TestCacheInsertUpdatesWeight(t *testing.T) {
	...
	if cache.Weight() != 2 {
		t.Fatal("WeightedCache with budget 2 did not correctly set weight after evicting one of three nodes")
	}
}
...
func TestCacheInsertDuplicateRejected(t *testing.T) {
	...
	if dupeError == nil {
		t.Fatal("WeightedCache insert of duplicate key did not result in any err")
	}
}
...
func TestCacheInsertEvictsLeastRecentlyUsed(t *testing.T) {
	...
	if foundEvicted {
		t.Fatal("WeightedCache insert did not trigger eviction after weight exceedance")
	}
	...
	if !foundA || !foundB {
		t.Fatal("WeightedCache insert evicted more than necessary")
	}
}
...
func TestCacheInsertEvictsLeastRecentlyRetrieved(t *testing.T) {
	...
	_, foundEvicted := cache.Retrieve("evicted")
	if foundEvicted {
		t.Fatal("WeightedCache insert did not evict least recently used after weight exceedance")
	}
}
